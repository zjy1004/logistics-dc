          ...
          <div class="form-con">
            <el-form :inline="true" ref="form" :model="form" class="demo-form-inline" label-width="100px">
              ...
              <el-form-item label="运输状态:">
                <el-select v-model="waybillTypeId" placeholder="请选择运输状态" clearable>
                  <el-option
                    v-for="(item, index) in waybillStatusOption"
                    :key="index"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  ...
                  <a class="edit" href="javascript:;" type="text" size="small" v-if="saveRoleType === 30 && scope.row.waybillStatus === 30 && scope.row.operateFlag === 0" @click="signIn(scope.row)">签收</a>
                  <a class="edit" href="javascript:;" type="text" size="small" v-if="saveRoleType === 30 && (scope.row.waybillStatus === 10 || scope.row.waybillStatus === 20) && scope.row.cancelFlag === 0" @click="cancel(scope.row)">取消</a>
                  <a class="enable" href="javascript:;" type="text" size="small" v-if="saveRoleType === 30 && scope.row.waybillStatus === 30 && scope.row.operateFlag === 0" @click="reject(scope.row)">拒收</a>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
export default {
  ...
  data () {
    return {
      saveRoleType: '',
      waybillStatusOption: [{id: '', name: '全部'}, {id: 10, name: '待发货'}, {id: 30, name: '运输中'}, {id: 40, name: '已签收'}, {id: 50, name: '已关闭'}], // 运输类型
      waybillTypeId: '',
      form: {
        appQueryFlag: 2,
        pkLine: '',
  ...
  watch: {
    waybillTypeId (newVal, oldVal) {
      if (newVal === '') {
        this.form.waybillStatus = []
      }
      if (newVal === 10) {
        this.form.waybillStatus = [10]
      }
      if (newVal === 30) {
        this.form.waybillStatus = [30]
      }
      if (newVal === 40) {
        this.form.waybillStatus = [40]
      }
      if (newVal === 50) {
        this.form.waybillStatus = [50]
      }
    }
  },
  methods: {
    ...
    queryList (type) {
      ...
      if (this.saveRoleType === 30) {
        ...
        WaybillManageAjax.QueryOcAndPfWbill(queryParam).then(response => {
          if (response.code === 200) {
            this.tableData = response.data.list.map((item, index) => {
              item.rowNum = index + 1
              return item
    ...
    cancel (val) { // 取消运单
      WaybillManageAjax.Cancel({waybillIdList: [val.waybillId]}).then(res => {
        if (res.code === 200) {
          this.queryList(this.form)
          this.$notify({
            type: 'success',
    ...
    // 查看
    check (row) {
      this.$router.push({name: 'WaybillDetail', query: {waybillTitle: '全部运单', waybillId: row.waybillId}})
    }
  }
}
</script>
