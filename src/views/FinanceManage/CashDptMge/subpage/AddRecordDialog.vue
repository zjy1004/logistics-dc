    ...
    <div>
      <el-form ref="form" :model="form" :rules="rules" class="username-tbody" label-width="120px">
        ...
        <el-form-item label="金额:" prop="amount">
          <el-input v-model="form.amount" @keyup.native="moneyChange($event, form.amount)" maxlength="7"  placeholder="请输入金额"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        tcId: [{ required: true, message: '请选择运力', trigger: 'blur' }],
        operateType: [{ required: true, message: '请选择类型', trigger: 'change' }],
        amount: [{ required: true, message: '请输入金额', trigger: 'blur' }],
        companyId: [{ required: true, message: '请选择物流公司', trigger: 'blur' }]
      },
  ...
  methods: {
    getTrans (name, type) { // 查询运力
      CashDptMgeAjax.GetTrans({tcName: name, scale: type}).then(res => {
        if (res.code === 200) {
