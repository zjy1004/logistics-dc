...
<script>
import FicAllWbillAjax from '@/api/FinanceManage/FicWbillMge/FicAllWbill'
...
export default {
  ...
  methods: {
    ...
    queryList (type) {
      ...
      FicAllWbillAjax.QueryList(queryParam).then(response => {
    ...
    queryLineList () { // 线路列表
      FicAllWbillAjax.QueryLineList().then(res => {
        if (res.code === 200) {
    ...
    exportExcel () {
      ...
      exportParams.url = '/dc/waybill/finance/list/export'
    ...
    // 查看
    check (row) {
      this.$router.push({name: 'FicWbillDetail', query: {waybillId: row.waybillId, waybillTitle: '全部运单'}})
    }
  }
}
</script>
