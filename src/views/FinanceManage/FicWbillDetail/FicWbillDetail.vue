<template>
  <div class="fic-wbill-detail public_card">
...
export default {
  ...
  created () {
    ...
    if (this.waybillTitle === '运费结算') {
      this.freightSettleObj.id = this.$route.query.id
      this.freightSettleObj.userType = this.$route.query.userType
      this.freightSettleObj.userName = this.$route.query.userName
      this.freightSettleObj.waybillCount = this.$route.query.waybillCount
      this.freightSettleObj.waybillQuantity = this.$route.query.waybillQuantity
      this.freightSettleObj.shiftRunCount = this.$route.query.shiftRunCount
      this.freightSettleObj.createTimeStart = this.$route.query.createTimeStart
      this.freightSettleObj.createTimeEnd = this.$route.query.createTimeEnd
    }
    if (this.waybillTitle === '待交款') {
      this.startDcObj.id = this.$route.query.id
      this.startDcObj.dcName = this.$route.query.dcName
      this.startDcObj.createTimeStart = this.$route.query.createTimeStart
      this.startDcObj.createTimeEnd = this.$route.query.createTimeEnd
    }
  ...
  methods: {
    ...
    goBack () {
      ...
      if (waybillTitle === '全部运单') {
        this.$router.push({name: 'FicAllWbill'})
      }
      if (waybillTitle === '已完结运单') {
        this.$router.push({name: 'FicFinishWbill'})
      }
      if (waybillTitle === '月结运费') {
        this.$router.push({name: 'MonPayFreight'})
      }
      if (waybillTitle === '运费结算') {
        this.$router.push({name: 'FreightSettleDetail', query: {id: this.freightSettleObj.id, userType: this.freightSettleObj.userType, waybillCount: this.freightSettleObj.waybillCount, waybillQuantity: this.freightSettleObj.waybillQuantity, shiftRunCount: this.freightSettleObj.shiftRunCount, userName: this.freightSettleObj.userName, createTimeEnd: this.freightSettleObj.createTimeEnd, createTimeStart: this.freightSettleObj.createTimeEnd}})
      }
      if (waybillTitle === '待交款') {
        this.$router.push({name: 'SettleForStartDcDetail', query: {id: this.startDcObj.id, dcName: this.startDcObj.dcName, createTimeEnd: this.startDcObj.createTimeEnd, createTimeStart: this.startDcObj.createTimeEnd}})
      }
    },
